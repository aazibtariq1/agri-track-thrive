...
const categories = [
  "seeds", "fertilizers", "pesticides", "labor", "machinery",
  "water", "electricity", "fuel", "maintenance", "other",
];
...
export default function Expenses() {
  ...
  const loadOfflineEntries = () => {
    setOfflineEntries(getQueue().filter(e => e.table === 'expenses'));
  };
  ...
  const loadExpenses = async () => {
    try {
      ...
      if (!user) return;
      const { data, error } = await supabase
        .from("expenses").select("*").eq("user_id", user.id).order("expense_date", { ascending: false });
      if (error) throw error;
      setExpenses(data || []);
    } catch (error: any) {
      console.error('Load expenses error:', error);
      toast({ title: "Error loading expenses", description: getUserFriendlyError(error), variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!isOnline) {
        ...
        toast({ title: "Saved offline", description: "Expense will sync when you're back online." });
        setOpen(false);
        resetForm();
        loadOfflineEntries();
        refreshPendingCount();
      ...
      if (isEditing) {
        ...
        if (error) throw error;
        toast({ title: "Expense updated successfully", description: `PKR ${formData.amount} expense updated.` });
      } else {
        ...
        if (error) throw error;
        toast({ title: "Expense added successfully", description: `PKR ${formData.amount} expense recorded.` });
      }
      ...
      toast({ title: isEditing ? "Error updating expense" : "Error adding expense", description: getUserFriendlyError(error), variant: "destructive" });
    }
  };
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (error) throw error;
      toast({ title: "Expense deleted", description: "Expense record removed successfully." });
      loadExpenses();
    } catch (error: any) {
      console.error('Delete expense error:', error);
      toast({ title: "Error deleting expense", description: getUserFriendlyError(error), variant: "destructive" });
    }
  };
  ...
  return (
    <Layout>
      <div className="space-y-6">
            ...
            <DialogContent>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <Button type="submit" className="w-full">
                  ...
                  {isEditing ? "Update Expense" : isOnline ? "Add Expense" : "Save Offline"}
                </Button>
              </form>
            </DialogContent>
          </Dialog>
                ...
                <TableBody>
                  {offlineEntries.map((entry) => (
                    <TableRow key={entry.id}>
                      <TableCell>{entry.data.expense_date}</TableCell>
                      <TableCell className="capitalize">{entry.data.category}</TableCell>
        ...
        {expenses.length === 0 && offlineEntries.length === 0 ? (
          <Card>
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            ...
                            <AlertDialogFooter>
                              ...
                              <AlertDialogAction onClick={() => handleDelete(expense.id)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">Delete</AlertDialogAction>
                            </AlertDialogFooter>
                          </AlertDialogContent>
                        </AlertDialog>
                      </TableCell>
